...
(check "autogenerated acccessors for def do not overwrite existing accessors"
  (let* ((ns   (gensym))
         (ns/x (intern (concat (symbol-name ns) "/x"))))
    (eval
     `(progn
        (namespace ,ns :export [x])
        (defn x () "Hand-written acessor" 'expected)
        (def x 'fail)
        (should (equal 'expected (,ns/x)))))))


(check "autogenerated acccessors for defmutable do not overwrite existing accessors"
  (let* ((ns   (gensym))
         (ns/x (intern (concat (symbol-name ns) "/x"))))
    (eval
     `(progn
